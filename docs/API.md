...
```json
{
  "artifact": {
    ...
    "download_url": "/api/v1/artifacts/artifact-0123456789abcdef0123456789abcdef",
...
Use `GET` or `HEAD /api/v1/artifacts/:id` while the same server process is
running. Bearer IDs contain 128 bits generated by the operating system's secure
random source and are not filenames. Downloads are asynchronously streamed in
bounded chunks with bounded concurrency. The response includes authoritative
`Content-Type`, `Content-Length`, safe `Content-Disposition`, and
`Cache-Control: private, no-store` headers.

Artifacts live only in a server-owned temporary directory. A graceful Ctrl-C
or SIGTERM drains the server and deletes that directory. An abrupt process or
machine termination cannot guarantee application cleanup and can leave an OS
temporary entry; there is no startup scavenger or retention guarantee. Public
...
| `quality` | integer | no | `image.default_quality` for JPEG | 1-100; accepted only for JPEG output |
| `format` | string | no | input format | `jpg`, `jpeg`, `png`, `webp`, `tiff`, `bmp`, `gif` |
...
curl -X POST http://127.0.0.1:8080/api/v1/image/compress \
  -F "file=@photo.jpg" -F "quality=85" -F "format=jpeg"
```
...
AVIF and ICO are recognized but return HTTP 501 before request files are
written because their REST encoders are not proven in Milestone 1. Other
...
(`webp_lossless`, `avif_enabled`, `max_dimension`, `jpeg_quality`,
`png_compression`, and `dither`) must retain their defaults; the REST server
fails startup when a non-default value would otherwise be silently ignored.
...
Messages are stable and sanitized; processor diagnostics and host paths are
never serialized. Safe machine-readable `details` are included only when
